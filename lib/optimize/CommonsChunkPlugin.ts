...
The available options are:
    ...
    normalizeOptions(options: CommonsChunkPlugin.Option | string | string[]): NormalizedOption {
    ...
    apply(compiler: Compiler) {
        compiler.plugin('this-compilation', (compilation: Compilation) => {
            compilation.plugin(['optimize-chunks', 'optimize-extracted-chunks'], (chunks: Chunk[]) => {
                 ...
                 * - If no chunk with the name as given in chunkNames exists a new chunk is created and added to the
                 * list
                 *
                ...
                targetChunks.forEach((targetChunk, idx) => {
                    ...
                    // with the commonChunk and get the origins for the asyncChunk
                    // (remember "asyncChunk === commonChunk" at this moment). bail out
                    if (this.async) {
...
Take a look at the "name"/"names" or async/children option.`);
    ...
    createAsyncChunk(compilation: Compilation, asyncOption: boolean, targetChunk: Chunk) {
    ...
    getModuleFilter(minChunks: MinChunksFilter, targetChunk: Chunk, usedChunksLength: number): ModuleFilter {
        if (typeof minChunks === 'function') {
            return minChunks;
        }
        const minCount = (minChunks || Math.max(2, usedChunksLength));
        const isUsedAtLeastMinTimes = (module: Module, count: number) => count >= minCount;
        return isUsedAtLeastMinTimes;
    }

    getExtractableModules(minChunks: MinChunksFilter, usedChunks: Chunk[], targetChunk: Chunk): Module[] {
    ...
    extractModulesAndReturnAffectedChunks(reallyUsedModules: Module[], usedChunks: Chunk[]): Set<Chunk> {
    ...
    moveExtractedChunkBlocksToTargetChunk(chunks: Set<Chunk>, targetChunk: Chunk) {
    ...
    extractOriginsOfChunksWithExtractedModules(chunks: Set<Chunk>) {
        const origins = [];
        for (const chunk of chunks) {
...
interface ModuleFilter {
    (module: Module, count: number): boolean
}

type MinChunksFilter = number | ModuleFilter
